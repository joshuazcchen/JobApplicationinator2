...
export function registerDatabaseIPC(): void {
	...
	ipcMain.handle("kw:create", (_e, name: string, triggers: string[]) => KW.createKeyword(db, name, triggers));
	...
	ipcMain.handle("kw:addBlurb", (_e, keywordId: number, label: string, contentHtml: string, isDefault: boolean) =>
	...
	ipcMain.handle("tpl:create", (_e, name: string, contentHtml: string) => TPL.createTemplate(db, name, contentHtml));
	...
	ipcMain.handle("app:updateStatus", (_e, id: number, status: string) => APP.updateApplicationStatus(db, id, status));
	ipcMain.handle("app:updateNotes", (_e, id: number, notes: string) => APP.updateApplicationNotes(db, id, notes));
	ipcMain.handle("app:saveCoverLetter", (_e, id: number, html: string) => APP.saveCoverLetter(db, id, html));
}
